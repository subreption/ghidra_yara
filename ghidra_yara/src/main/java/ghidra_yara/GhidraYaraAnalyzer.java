...
public class GhidraYaraAnalyzer extends AbstractAnalyzer
{
	...
	private void handleMatch(Program program, AddressSetView set, TaskMonitor monitor, MessageLog log,
			YaraRule rule, Address baseAddress) throws CancelledException
	{
		...
		for (YaraString yaraString : IterableHelper.toIterable(rule.getStrings()))
		{
			...
			for (YaraMatch curMatch : IterableHelper.toIterable(yaraString.getMatches()))
			{
				...
				String idStr = String.format("YARA_%s_%s", rule.getIdentifier(),
				...
				try {
					...
					if (addBookmarks) {
						bookmarkManager.setBookmark(matchAddress,
			...
			throws CancelledException
	{
		...
		try {
			...
			Msg.info(this, "Finished scanning.");
